...
bool TuyaModuleSelected(void)
{
  ...
  if (!(pin[GPIO_TUYA_RX] < 99) || !(pin[GPIO_TUYA_TX] < 99 || pin[GPIO_TUYA_TXS] < 99)) { 
...
void TuyaInit(void)
{
  ...
  if (Tuya.buffer != nullptr) {
    ...
    TuyaSerial = new TasmotaSerial(pin[GPIO_TUYA_RX], (sink_tx) ? pin[GPIO_TUYA_TXS] : pin[GPIO_TUYA_TX], 2, 0, TM_SERIAL_BUFFER_SIZE, sink_tx);
