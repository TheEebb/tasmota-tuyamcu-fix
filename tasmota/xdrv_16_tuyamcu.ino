...
bool TuyaCommWaitReceiveBeforeSend(struct TUYA_TX_BLOCK& block) {
  ...
  bool result = TuyaSerialInput((int)block.timeout * (int)sleep / (int)TUYA_RX_WAIT_TIME);
...
inline uint8_t TuyaCalcTimeoutCycles(uint timeout_ms) {
  uint loops = (timeout_ms / (uint)sleep) + 1; // Add an additional loop because 0 = unlimited
...
bool TuyaStartModeChange(uint8_t esp_mode) {
  ...
  if (mcu_stage < last_stage && TuyaProcessSendCommandFirst(mcu_cmd)) {
    ...
    sleep = TUYA_INIT_SLEEP;    // Temporarily change main loop sleep during mode change sequence
...
{
  if (pin[GPIO_TUYA_TX] < 99 && pin[GPIO_TUYA_TXS] < 99) {
    // reset configuration if both tx and tx sink are selected
    Settings.my_gp.io[pin[GPIO_TUYA_TXS]] = 0;
    pin[GPIO_TUYA_TXS] = 99;
    restart_flag = 2;
  }

  if (pin[GPIO_TUYA_RX] < 99 && pin[GPIO_TUYA_RXS] < 99) {
    // reset configuration if both tx and tx sink are selected
    Settings.my_gp.io[pin[GPIO_TUYA_RXS]] = 0;
    pin[GPIO_TUYA_RXS] = 99;
    restart_flag = 2;
  }

  if (!(pin[GPIO_TUYA_RX] < 99 || pin[GPIO_TUYA_RXS] < 99) || !(pin[GPIO_TUYA_TX] < 99 || pin[GPIO_TUYA_TXS] < 99)) { 
    // fallback to hardware-serial if not explicitly selected
    pin[GPIO_TUYA_TX] = 1;
    pin[GPIO_TUYA_RX] = 3;
...
void TuyaInit(void)
{
  ...
  if (Tuya.buffer != nullptr && Tuya.queue.blocks != nullptr) {
    TuyaQueueReset();
    bool pullup_rx = (pin[GPIO_TUYA_RXS] < 99);
    bool sink_tx = (pin[GPIO_TUYA_TXS] < 99);
    TuyaSerial = new TasmotaSerial(
      (pullup_rx) ? pin[GPIO_TUYA_RXS] : pin[GPIO_TUYA_RX],
      (sink_tx) ? pin[GPIO_TUYA_TXS] : pin[GPIO_TUYA_TX],
      2,
      0,
      TM_SERIAL_BUFFER_SIZE,
      ((pullup_rx)?TM_SERIAL_RX_PULL_UP:0) | ((sink_tx)?TM_SERIAL_TX_OPEN_DRAIN:0)
    );
    if (TuyaSerial->begin(TUYA_BAUD_RATE)) {
...
bool TuyaSerialTryReadPacket()
{
  while (TuyaSerial->available()) {
    ...
    if (TuyaSerial->hasRxError())
      AddLog_P(LOG_LEVEL_DEBUG, PSTR("TYA: Serial RX has error!"));   // <--- testing - maybe restart packet routine
